...
public class RestaurantService {
	...
	public int getReportCount(int rm_num) {
		return reportDao.selectRestaurant(rm_num);
	}

	public void updateResImg(int r_num, String src) {
		HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("r_num", r_num);
		map.put("r_photo", src);
		restaurantDao.updateResImg(map);
	}
	...
	// 리뷰 쓰기
	public int Review_Write(List<MultipartFile> multi, List<String> delimg, String rev_num, int r_num, int m_num,
			String textarea, double grade) {
		String root = System.getProperty("user.dir") + "/target/classes/static/";
		if (delimg != null) {
		...
		if (multi != null) {
			...
			for (MultipartFile image : multi) {
				String originalName = image.getOriginalFilename();
				String safeFile = path + System.currentTimeMillis() + i + "."
				...
				case XmlPullParser.START_TAG:
					...
					case "mapy":
						...
						try {
							...
							map.put("r_name", (String) restaurant_map.get("r_name"));
							map.put("r_address", (String) restaurant_map.get("r_address"));
							String r_photo = restaurantDao.selectOneRestaurantName(map).getR_photo();
							restaurant_map.put("r_photo", r_photo);
	...
	public HashMap<String, Object> getBoardListPageAdminKeyword(int page, String keyword) {
		...
		HashMap<String, Object> pageInfo = new HashMap<String, Object>();
		/*
		 * pageInfo.put("page", page); // pageInfo.put("current", page);
		 ...
		 * restaurantDao.getCount()); pageInfo.put("keyword", keyword);
		 */
		if (keyword != null) {
			// 페이징 처리 정보를 담은 맵
			// HashMap<String, Object> pageInfo = new HashMap<String, Object>();
	...
	public HashMap<String, Object> RestaurantSelect(HashMap<String, String> data) {
		...
		try {
			...
			RestModify rm = restModifyDao.selectModify(r_num);

			// 신고상태 확인
			map.put("report", reportDao.selectRestaurant(rm.rm_num));
	...
	public boolean View_Report(HashMap<String, String> map) {
		Report r = reportDao.memberCheck(map);
		if (r == null) {
			Report report = new Report(0, Integer.parseInt(map.get("rm_num")), Integer.parseInt(map.get("m_mail1")),
					Integer.parseInt(map.get("m_mail2")), Integer.parseInt(map.get("replist_num")), 1, new Date(),
					null);
